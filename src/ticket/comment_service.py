...
class TicketCommentService:
  ...
  async def add_comment(
    ...
    if current_user.role == Role.CITIZEN:
      ...
      if not await TicketWorkflowService.can_view_internal_ticket(
        db,
        ticket,
        current_user,
      ):
  ...
  async def list_comments(
    ...
    if current_user is not None and current_user.role in {
      ...
      include_internal = await TicketWorkflowService.can_view_internal_ticket(
        db,
        ticket,
        current_user,
      )
    ...
    elif not await TicketService._can_view_ticket(db, ticket, current_user):
      raise ResourceNotFoundException("Ticket not found", error_code="TICKET_NOT_FOUND")
