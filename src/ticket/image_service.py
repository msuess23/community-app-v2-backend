...
class TicketImageService:
  ...
  async def _require_manage_permission(
    ...
    if not await TicketWorkflowService.can_view_internal_ticket(db, ticket, current_user):
  ...
  async def remove_image(
    ...
    removed_event = await TicketService._append_event(
      db,
      ticket,
      actor_user_id=current_user.id,
    ...
    if remaining and not any(item.is_cover for item in remaining):
      ...
      cover_event = await TicketService._append_event(
        db,
        ticket,
        actor_user_id=current_user.id,
        event_type=TicketEventType.TICKET_COVER_IMAGE_CHANGED,
  ...
  async def get_content(
    ...
    if image.is_active:
      if not await TicketService._can_view_ticket(db, ticket, current_user):
        raise ResourceNotFoundException(
          "Ticket image not found",
          error_code="TICKET_IMAGE_NOT_FOUND",
        )
